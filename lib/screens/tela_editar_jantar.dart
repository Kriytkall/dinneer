...
class _TelaEditarJantarState extends State<TelaEditarJantar> {
  ...
  void initState() {
    ...
    _vagasController = TextEditingController(text: widget.jantar.nuMaxConvidados.toString());
  ...
  void _salvarAlteracoes() async {
    ...
    try {
      ...
      if (res != null && (res['registros'] == 1 || res['dados'] != null)) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Jantar atualizado!")));
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text("Editar Jantar"), backgroundColor: Colors.white, foregroundColor: Colors.black, elevation: 0),
      body: SingleChildScrollView(
        ...
        child: Column(
          children: [
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  image: _novaImagem != null 
                    ? DecorationImage(image: FileImage(_novaImagem!), fit: BoxFit.cover)
                    : (widget.jantar.urlFoto != null 
                        ? DecorationImage(image: NetworkImage(widget.jantar.urlFoto!), fit: BoxFit.cover)
                        : null),
                ),
            ...
            CampoDeTextoCustomizado(controller: _tituloController, dica: "Título"),
